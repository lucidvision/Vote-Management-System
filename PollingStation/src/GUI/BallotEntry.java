...
public class BallotEntry extends JFrame implements ActionListener{
	private Controller ctlr;
	private ballotmanagement ballman;
	private pollingstation PollStat;
	
	private int initialWidth = 450;
	private int initialHeight = 530;
	Container content;
	JButton submit, finish;		JCheckBox spoiled;
	JTextField ballotID;			JComboBox[] cans;
	
	String[] can_party = null;	int num_cans;	
	String[] ranks;	
	
	ArrayDeque<ballot> ballotDeque;		int phase;
	ArrayDeque<Integer> ids;
	
	public BallotEntry( Controller con ,  int ph ) {
		ctlr = con;
		phase =  ph;
		ballman = con.getBallMan();
		PollStat = con.getPollStat();
	...
	private JPanel getTop(){
		...
		top.add(new JLabel("<html><left>After all ballots have been submitted "+
				"press finish poll button<br>to commit your entries.</html>"),BorderLayout.CENTER);
	...
	private JPanel getButtons(){
		...
		finish = new JButton("Finish Poll");	finish.addActionListener(this);
		
		JPanel buttons = new JPanel(new GridLayout());
		GridBagConstraints e = new GridBagConstraints();
		e.gridx = 0;	e.gridy = 0;	buttons.add(submit,e);
		e.gridx++;	buttons.add(finish,e);
	...
	public void actionPerformed(ActionEvent a) {
		if(a.getSource() == submit){
			if(toDeque()){clearBallot();}
		} else if(a.getSource() == finish){
			if(0 == JOptionPane.showConfirmDialog(this,
					"<html>Please confirm that you would<br>"
					+"like to commit all " + ballotDeque.size() + " ballots entered.")){
				ballman.submit( ballotDeque, phase );
				new ROMainMenu( ctlr );
				this.dispose();	
			}
		}		
